...
/**
 * Fetches an image using a robust CORS proxy fallback cascade.
 *
...
/**
 * Inserts a compressed image with native Amplenote caption syntax (> Caption immediately on the next line).
 *
 ...
 * @param {string} [auditInfo] - Caption or audit info
 * @returns {string} Updated markdown content
 */
export function insertImageBelow(content, originalSrc, newSrc, auditInfo = "Compressed") {
    if (!content || !originalSrc || !newSrc) return content || "";

    // Escape special regex characters in originalSrc
    const escapedSrc = originalSrc.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

    // Matches the image tag AND any immediate caption line that belongs to it:
    // ![filename|width](originalSrc)
    // > Existing Caption
    const regex = new RegExp(`(!\\[[^\\]]*\\]\\(${escapedSrc}\\)(?:\\r?\\n>[^\\r\\n]*)?)`, "g");

    // In Amplenote markdown, caption MUST be immediately on the next line (single \n>, not \n\n>)
